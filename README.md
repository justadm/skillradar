...
- `GET /suggests/skill_set?text=...` — подсказки по навыкам (актуальный endpoint для поиска навыков).
...
- `HH_CLIENT_ID`
- `HH_CLIENT_SECRET`
- `HH_REDIRECT_URI` (например `https://hr.devee.ru/oauth/hh/callback`)
...
- `SMSC_WEBHOOK_KEY` — секретный ключ для webhook статусов SMSC (`/api/v1/integrations/smsc/webhook?key=...`).
- `SMSC_ALERT_STATUSES` — статусы SMSC для алертов в OPS (по умолчанию `DELIVRD,UNDELIV`).
- `OAUTH_YANDEX_CLIENT_ID` / `OAUTH_YANDEX_CLIENT_SECRET` / `OAUTH_YANDEX_REDIRECT_URI` — вход через Яндекс ID.
- `OAUTH_SBER_CLIENT_ID` / `OAUTH_SBER_CLIENT_SECRET` / `OAUTH_SBER_REDIRECT_URI` — вход через Сбер ID.
- `OAUTH_*_AUTH_URL` / `OAUTH_*_TOKEN_URL` / `OAUTH_*_USERINFO_URL` — override URL провайдера (для кастомных стендов).
...
Сервисы:
- `web` — SSR/API (внутри контейнера 3000, на хосте по умолчанию `127.0.0.1:13001`)
- `frontend` — статика `/web` (ЛК/админка legacy, на хосте `127.0.0.1:13080`)
- `bot` — Telegram‑бот (без web, `DISABLE_WEB=true`)
- `nginx` — optional edge‑proxy контейнер (profile `standalone-edge`, по умолчанию `127.0.0.1:18088`)
...
Для прода с существующим хостовым nginx (много проектов на сервере):
...
```bash
docker build -f Dockerfile.runtime -t gridai-runtime .
```
...
```bash
sudo cp deploy/docker-logs.conf /etc/logrotate.d/gridai-docker
sudo logrotate -f /etc/logrotate.d/gridai-docker
```
...
ALERT_INTERVAL=60
ALERT_BOT_CONTAINER=gridai-bot
ALERT_NGINX_CONTAINER=gridai-nginx
ALERT_ON_5XX=true
```

### Systemd (docker compose)
Шаблон юнита: `deploy/gridai-docker.service`
```bash
sudo cp deploy/gridai-docker.service /etc/systemd/system/gridai-docker.service
sudo systemctl daemon-reload
sudo systemctl enable gridai-docker
sudo systemctl start gridai-docker
sudo systemctl status gridai-docker
```
...
```bash
sudo cp /opt/gridai/deploy/gridai.service /etc/systemd/system/gridai.service
sudo systemctl daemon-reload
...
- `POST /api/v1/auth/sign-in` и `POST /api/v1/auth/signin` → alias для login
- `POST /api/v1/auth/verify` → установит httpOnly cookie‑сессию
- `GET /api/v1/auth/oauth/providers` → список доступных OAuth провайдеров
- `GET /api/v1/auth/oauth/:provider/start` → старт OAuth (`yandex`, `sber`)
- `GET /api/v1/auth/oauth/:provider/callback` → callback OAuth
- `POST /api/v1/integrations/smsc/webhook?key=...` → webhook статусов/входящих SMS от SMSC (ответ `OK`)
  - при статусах из `SMSC_ALERT_STATUSES` отправляется алерт в OPS Telegram/email.
